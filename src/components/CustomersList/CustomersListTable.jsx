...
import {
  faSearch, faEye, faEdit, faTrashAlt, faSort, faSortUp, faSortDown, faTimes, faPlusCircle,
  faExclamationTriangle, faCheckCircle, faUser, faAddressCard, faPhone, faTasks, faCalendarAlt,
  faDollarSign, faSpinner, faDownload, faChevronDown, faChevronUp,
} from '@fortawesome/free-solid-svg-icons';
import styles from './CustomersList.module.css';

export default function CustomersListTable({
  searchQuery, setSearchQuery, paginatedCustomers, totalPages, currentPage, setCurrentPage,
  isLoading, error, lastUpdated, totals, notifications, isNotificationsOpen, setIsNotificationsOpen,
  handleSort, handleDetails, handleEdit, handleDelete, handleNewProject, handleExportCSV,
  sortConfig, formatPhoneNumber, formatDate, navigate,
}) {
  ...
  // Calculate payment progress percentage, removing unused 'paid'
      ...
      {isLoading ? (
        ...
        <div className={styles.tableWrapper}>
          ...
          <table className={styles.table}>
            <thead>
              <tr>
                <th scope="col"><FontAwesomeIcon icon={faUser} /> First</th>
                <th scope="col" onClick={() => handleSort('lastName')} className={styles.sortable}>
                  <FontAwesomeIcon icon={faAddressCard} /> Last <FontAwesomeIcon icon={getSortIcon('lastName')} />
                </th>
                <th scope="col"><FontAwesomeIcon icon={faPhone} /> Phone</th>
                <th scope="col"><FontAwesomeIcon icon={faTasks} /> Projects</th>
                <th scope="col" onClick={() => handleSort('startDate')} className={styles.sortable}>
                  <FontAwesomeIcon icon={faCalendarAlt} /> Start <FontAwesomeIcon icon={getSortIcon('startDate')} />
                </th>
                <th scope="col"><FontAwesomeIcon icon={faCalendarAlt} /> Finish</th>
                <th scope="col">Status</th>
                <th scope="col" onClick={() => handleSort('amountRemaining')} className={styles.sortable}>
                  <FontAwesomeIcon icon={faDollarSign} /> Remaining <FontAwesomeIcon icon={getSortIcon('amountRemaining')} />
                </th>
                <th scope="col"><FontAwesomeIcon icon={faDollarSign} /> Total</th>
              ...
              {paginatedCustomers.map((customer) => {
                ...
                return (
                  <tr key={`${customer.customerInfo.lastName}-${customer.customerInfo.phone}`}>
                    ...
                    <td className={customer.totalAmountRemaining > 0 ? styles.amountDue : styles.amountPaid}>
                      ${customer.totalAmountRemaining.toFixed(2)} ({progress}% Paid)
                    </td>
                    ...
                    <td className={styles.actions}>
                      ...
                      <button onClick={() => handleNewProject(customer.customerInfo)} className={`${styles.actionButton} ${styles.newProjectButton}`} title="Add New Project">
        ...
        <p className={styles.noResults}>
          No customers found. <button onClick={() => navigate('/home/customer')} className={styles.inlineButton}>Add a new customer</button>.
        </p>
      )}
    </>
  );
